...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupRemainingSpots() {
        ...
        // Réduire aléatoirement le nombre de places si c'est un nouveau visiteur
        const visitorKey = 'enorehab_visitor';
        if (!sessionStorage.getItem(visitorKey)) {
            // Marquer comme visité
            sessionStorage.setItem(visitorKey, 'true');
            
            // 50% de chance de réduire le nombre de places
            if (Math.random() > 0.5 && remainingSpots > 3) {
                remainingSpots -= 1;
                localStorage.setItem(storageKey, remainingSpots);
            }
        }
        ...
        if (remainingSpots <= 3) {
            ...
            if (pricingSection) {
                ...
                urgencyAlert.className = 'mt-4 text-red-500 font-bold text-center animate-pulse';
    ...
    function setupCTAAttention() {
        ...
        // Fonction pour faire trembler légèrement un bouton
        function shakeCTA(button) {
            // Créer l'animation de tremblement
            button.animate([
        ...
        // Animer chaque CTA après un certain délai
        ctaButtons.forEach((button, index) => {
            setTimeout(() => {
                // Faire trembler le bouton
                shakeCTA(button);
                
                // Répéter l'animation aléatoirement
                setInterval(() => {
                    if (Math.random() > 0.7) { // 30% de chance d'animer
                        shakeCTA(button);
                    }
                }, 12000); // Vérifier toutes les 12 secondes
                
            }, 3000 + (index * 500)); // Délai initial différent pour chaque bouton
        });
    }
